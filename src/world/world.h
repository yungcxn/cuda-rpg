...
#define WID(nid) _WORLD_ID_##nid


#define X_WORLDLIST \
/*      X_WORLD_TUPLE(name_id, loadfunc, updatefunc) */ \
        X(WID(MAIN), worldmain_load, worldmain_update) \
        X(WID(HOME), worldhome_load, worldhome_update)

#define X(name_id, loadfunc, updatefunc) name_id,
typedef enum {
        X_WORLDLIST
        WORLD_COUNT
} world_id_t;
