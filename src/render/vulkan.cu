...
static VkExtent2D render_extent = {WIDTH, HEIGHT};
...
static inline void _create_swapchain_and_shared_image(void) {
        ...
        VkSurfaceFormatKHR* formats = (VkSurfaceFormatKHR*) malloc(sizeof(VkSurfaceFormatKHR)
                                                                   * fmt_count);
...
tex_realrgba_t** vulkan_setup(void) {
        ...
        if (vkCreateInstance(&ic, 0, &instance) != VK_SUCCESS) 
        ...
        VkPhysicalDevice* devices = (VkPhysicalDevice*) malloc(sizeof(VkPhysicalDevice) 
                                                               * dev_count);
        ...
        if (vkCreateDevice(phys_dev, &dci, 0, &device) != VK_SUCCESS) 
...
void vulkan_pre_render(uint32_t* image_index) {
        ...
        VkResult r = vkAcquireNextImageKHR(
                device, swapchain, UINT64_MAX, VK_NULL_HANDLE, VK_NULL_HANDLE, image_index
        );
...
void vulkan_post_render(uint32_t image_index) {
        ...
        vkCmdPipelineBarrier(
                cmd, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT,
                0, 0, 0, 0, 0, 1, &barrier2
        );
        
        /* 0x0A080DFF */
        VkClearColorValue clear_color = {{10.0f / 255.0f, 8.0f / 255.0f, 13.0f / 255.0f, 1.0f}};
        ...
        vkCmdClearColorImage(
                cmd, swapchain_images[image_index], VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 
                &clear_color, 1, &range
        );
        ...
        VkImageBlit blit_region = {
                ...
                .dstOffsets = {
                        {render_offset.x,
                         render_offset.y, 0},
                        {render_offset.x + (int32_t)render_extent.width,
                         render_offset.y + (int32_t)render_extent.height, 1}
                }
        };
        vkCmdBlitImage(
                cmd, shared_image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, 
        ...
        VkResult res = vkQueuePresentKHR(graphics_queue, &pinfo);
